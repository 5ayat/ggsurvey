...
```{r, message = FALSE}
tab <- svyby(~i_outdoors, ~prs, dstrat, svymean, na.rm=TRUE)
...
```{r, message = FALSE}
tab <- svyby(~i_outdoors, ~camp+gender, dstrat, svymean, na.rm=TRUE)
...
```{r, message = FALSE, warning=FALSE}
tab <- svyby(~i_outdoors, ~gender, dstrat, svymean, na.rm=TRUE)
gg.svyby(tab, color = pal, percent = TRUE, legend.title = "Importance") + 
  ggtitle('\nImportance of Outdoor Activities\n') + 
...
tab <- svyby(~i_outdoors, ~camp, dstrat, svymean, na.rm=TRUE)
gg.svy.likert(tab, color=pal, legend.position = 'right', order=TRUE, line = 9) +
  ggtitle('\nLikert Plot of Importance of Outdoors\n')
```

```{r, warning=FALSE,message=FALSE}
tab <- svyby(~i_outdoors, ~s_outdoors, dstrat, svymean, na.rm=TRUE)
